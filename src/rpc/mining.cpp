...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or bitzec.conf setting gen)\n"
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in Satoshis)\n"
...
#else
        throw JSONRPCError(RPC_METHOD_NOT_FOUND, "bitzecd compiled without wallet and -mineraddress not set");
#endif
    }
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Bitzec is not connected!");

    if (IsInitialBlockDownload(Params()))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Bitzec is downloading blocks...");
