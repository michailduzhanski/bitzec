...
Note that a backup is a duplicate of data needed to spend ARK so where you keep your backup(s) is another important consideration. You should not store backups where they would be equally or increasingly susceptible to loss or theft.

## Instructions for backing up your wallet and/or private keys

These instructions are specific for the officially supported Arnak Linux client. For backing up with third-party wallets, please consult with user guides or support channels provided for those services.
...
For all methods, you will need to include an export directory setting in your config file (`arnak.conf` located in the data directory which is `~/.arnak/` unless it's been overridden with `datadir=` setting):
...
Note that arnakd will need to be stopped and restarted for edits in the config file to take effect.

### Using `backupwallet`

To create a backup of your wallet, use:

`arnak-cli backupwallet <nameofbackup>`.
...
If you generate a new Arnak address, it will not be reflected in the backup file.
...
`arnak-cli z_exportwallet <nameofbackup>`
...
`arnak-cli z_importwallet <path/to/exportdir/nameofbackup>`
...
`arnak-cli z_exportkey <z-address>`
...
`arnak-cli dumpprivkey <t-address>`

This will return the private key and will not create a new file.

To import a private key for a shielded address, use:

`arnak-cli z_importkey <z-priv-key>`
...
`arnak-cli z_importkey <z-private-key> no`
...
`arnakcli importprivkey <t-priv-key>`
